<?php
/**
 * Trust Wallet Provider Integration
 */
...
class Crypto_Exchange_Trust_Wallet {
    ...
    public function __construct($provider = null) {
        if ($provider) {
            ...
            $this->api_url = $provider->rpc_url ?: 'https://api.trustwallet.com';
            $this->api_key = $provider->api_key;
        }
    }
    
    /**
     * Test connection to Trust Wallet API
     */
    public function test_connection() {
        try {
            $response = $this->make_request('GET', '/v1/coins');
            
            if ($response && isset($response['coins'])) {
                return array(
                    'success' => true,
                    'message' => 'Trust Wallet connection successful. ' . count($response['coins']) . ' coins available.'
                );
            } else {
                return array(
                    'success' => false,
                    'message' => 'Invalid response from Trust Wallet API'
                );
            }
        } catch (Exception $e) {
            return array(
    ...
    public function create_address($user_id, $coin) {
        try {
            $address = $this->generate_address($coin);
    ...
    public function send_transaction($user_id, $to_address, $amount, $coin) {
        try {
            ...
            $transaction = $this->create_transaction($from_address, $to_address, $amount, $coin);
    ...
    /**
     * Make request to Trust Wallet API
     */
     ...
     */
    private function generate_address($coin) {
     ...
     * Create transaction
     */
    private function create_transaction($from_address, $to_address, $amount, $coin) {
        // This would typically create a real transaction
    ...
    private function get_user_addresses($user_id) {
        ...
        $addresses = $wpdb->get_col($wpdb->prepare(
            "SELECT address FROM {$wpdb->prefix}crypto_wallet_addresses 
             WHERE user_id = %d AND provider = 'trust_wallet'",
            $user_id
        ));
    ...
    private function get_user_primary_address($user_id, $coin) {
        ...
        $address = $wpdb->get_var($wpdb->prepare(
            "SELECT address FROM {$wpdb->prefix}crypto_wallet_addresses 
             WHERE user_id = %d AND provider = 'trust_wallet' AND coin = %s
